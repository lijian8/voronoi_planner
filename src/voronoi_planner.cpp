...
void VoronoiPlanner::destroy() {
  ...
  if (m_SearchSpace) {
    for (int x = 0; x < m_Width; x++) delete[] m_SearchSpace[x];
...
bool VoronoiPlanner::search(int startX, int startY, int goalX, int goalY,
                            int& pathCost,
                            std::vector<std::pair<int, int>>* path, int size_x,
                            int size_y, VoronoiData** voronoiDiagram,
  ...
  if (!SearchShortestPathToVoronoi(startX, startY, goalX, goalY, voronoiStartX,
                                   voronoiStartY, pathCost1, &path1,
  ...
  if (voronoiStartX == goalX && voronoiStartY == goalY) {
    path->clear();
    for (int i = 0; i < (int)path1.size(); i++) path->push_back(path1[i]);
  ...
  if (!SearchShortestPathToVoronoi(goalX, goalY, startX, startY, voronoiGoalX,
                                   voronoiGoalY, pathCost3, &path3,
                                   voronoiDiagram, circumscribed_radius)) {
  ...
  if (voronoiGoalX == startX && voronoiGoalY == startY) {
    path->clear();
    for (int i = 0; i < (int)path3.size(); i++) path->push_back(path3[i]);
  ...
  if (!SearchInVoronoi(voronoiStartX, voronoiStartY, voronoiGoalX, voronoiGoalY,
                       pathCost2, &path2, voronoiDiagram,
...
bool VoronoiPlanner::SearchInVoronoi(int startX, int startY, int goalX,
                                     int goalY, int& pathCost,
  ...
  if (use_heuristic_) key = key + heuristic(startX, startY, goalX, goalY);
  ...
  while (!m_OPEN->emptyheap() && searchGoalState->g > m_OPEN->getminkeyheap()) {
    ...
    for (int dir = 0; dir < 8; dir++) {
      ...
      if (!WithinSearchSpace(new_x, new_y)) continue;

      if (pbClosed[new_x + m_Width * new_y] == 1) continue;

      if (!voronoiDiagram[new_x][new_y].isVoronoi) continue;

      double dist = voronoiDiagram[new_x][new_y].dist;
      if (dist < circumscribed_radius) continue;
      ...
      if (searchSuccState->g > searchExpState->g + cost) {
        ...
        if (use_heuristic_) key = key + heuristic(new_x, new_y, goalX, goalY);
  ...
  //     m_SearchSpace[m_GoalX][m_GoalY].g, largestcomputedoptf_, m_StartX,
...
bool VoronoiPlanner::SearchShortestPathToVoronoi(
    int startX, int startY, int goalX, int goalY, int& voronoiGoalX,
    int& voronoiGoalY, int& pathCost, std::vector<std::pair<int, int>>* path,
  ...
  while (!m_OPEN->emptyheap()) {
    ...
    if ((exp_x == goalX && exp_y == goalY) ||
        (voronoiDiagram[exp_x][exp_y].isVoronoi)) {
      voronoiGoalX = exp_x;
      voronoiGoalY = exp_y;
      break;
    }
  ...
  path->clear();
  Grid2DSearchState* searchGoalState =
      &m_SearchSpace[voronoiGoalX][voronoiGoalY];
  Grid2DSearchState* searchPredState = searchGoalState;
  path->push_back(std::make_pair(searchPredState->x, searchPredState->y));
  while (searchPredState->predecessor != NULL) {
    searchPredState = searchPredState->predecessor;
