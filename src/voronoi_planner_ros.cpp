...
VoronoiPlannerROS::VoronoiPlannerROS()
    : voronoi_planner_(NULL),
      initialized_(false),
      costmap_ros_(NULL),
      size_x_(0),
      size_y_(0),
      voronoi_diagram_(NULL) {}
...
VoronoiPlannerROS::~VoronoiPlannerROS() {
  if (voronoi_planner_) delete voronoi_planner_;
...
void VoronoiPlannerROS::destroy() {
  if (voronoi_diagram_) {
    for (int i = 0; i < size_x_; i++) delete[] voronoi_diagram_[i];
...
bool VoronoiPlannerROS::makePlan(
  ...
  if (!voronoi_planner_->search(start_x, start_y, goal_x, goal_y, pathCost,
                                &path, size_x_, size_y_, voronoi_diagram_,
